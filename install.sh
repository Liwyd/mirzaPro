...
function show_menu() {
    ...
    print_menu_option "3)" "Remove mirzaPro Bot"
...
function install_bot() {
    echo -e "\e[32mInstalling mirzaPro script ... \033[0m\n"
...
sudo chmod -R 755 "$BOT_DIR"
    ...
    echo -e "\n\033[33mmirzaPro config and script have been installed successfully.\033[0m"
...
function install_bot_with_marzban() {
    echo -e "\033[41m[IMPORTANT WARNING]\033[0m \033[1;33mMarzban panel is detected on your server. Please make sure to backup the Marzban database before installing mirzaPro Bot.\033[0m"
    read -p "Are you sure you want to install mirzaPro Bot alongside Marzban? (y/n): " confirm
    ...
    if [ "$DB_TYPE" != "mysql" ]; then
        echo -e "\e[91mError: Your database is $DB_TYPE. To install mirzaPro Bot, you must use MySQL.\033[0m"
    ...
    clear
    echo -e "\e[33mConfiguring mirzaPro Bot database credentials...\033[0m"
...
function update_bot() {
    ...
    BOT_DIR="/var/www/html/mirzabotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[91mError: mirzaPro Bot is not installed. Please install it first.\033[0m"
        exit 1
    fi
    ...
    if [ -n "$INSTALL_SCRIPT_PATH" ]; then
        ...
        RAW_INSTALL_URL="https://raw.githubusercontent.com/Liwyd/mirzaPro/main/install.sh"
    ...
    echo -e "\n\e[92mmirzaPro Bot updated to latest version successfully!\033[0m"
...
function remove_bot() {
    echo -e "\e[33mStarting mirzaPro Bot removal process...\033[0m"
    LOG_FILE="/var/log/remove_bot.log"
    echo "Log file: $LOG_FILE" > "$LOG_FILE"

    BOT_DIR="/var/www/html/mirzabotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m mirzaPro Bot is not installed (/var/www/html/mirzabotconfig not found)." | tee -a "$LOG_FILE"
        echo -e "\e[33mNothing to remove. Exiting...\033[0m" | tee -a "$LOG_FILE"
        sleep 2
        exit 1
    fi

    read -p "Are you sure you want to remove mirzaPro Bot and its dependencies? (y/n): " choice
    if [[ "$choice" != "y" ]]; then
    ...
    echo "Removing mirzaPro Bot..." | tee -a "$LOG_FILE"
    ...
    echo -e "\e[92mmirzaPro Bot, MySQL, and their dependencies have been completely removed.\033[0m" | tee -a "$LOG_FILE"
}

function remove_bot_with_marzban() {
    echo -e "\e[33mRemoving mirzaPro Bot alongside Marzban...\033[0m" | tee -a "$LOG_FILE"
    ...
    echo -e "\e[92mmirzaPro Bot has been removed alongside Marzban. SSL certificates remain intact.\033[0m" | tee -a "$LOG_FILE"
}
...
function auto_backup() {
    ...
    BOT_DIR="/var/www/html/mirzabotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\033[31m[ERROR]\033[0m mirzaPro Bot is not installed ($BOT_DIR not found)."
...
function manage_additional_bots() {
    if [ ! -d "/var/www/html/mirzabotconfig" ]; then
        echo -e "\033[31m[ERROR]\033[0m The main mirzaPro Bot is not installed (/var/www/html/mirzabotconfig not found)."
...
EOF"
    ...
    git clone https://github.com/Liwyd/mirzaPro.git "$BOT_DIR" || {
...
function update_additional_bot() {
    ...
    if ! git clone https://github.com/Liwyd/mirzaPro.git "$BOT_PATH"; then
